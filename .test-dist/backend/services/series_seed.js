...
exports.orderDivisionsForSeeding = orderDivisionsForSeeding;
exports.computeSeededAssignments = computeSeededAssignments;
function orderDivisionsForSeeding(divisions) {
    return [...divisions].sort((a, b) => (b.sortKey ?? 0) - (a.sortKey ?? 0) || a.name.localeCompare(b.name));
}
function computeSeededAssignments(rawDivisions, advanceAmount, demoteAmount) {
    ...
    for (let index = 0; index < orderedDivisions.length; index += 1) {
        ...
        const demotedTeamIds = index < orderedDivisions.length - 1
